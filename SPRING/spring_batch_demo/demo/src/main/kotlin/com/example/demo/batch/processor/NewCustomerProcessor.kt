...
/**
 * NEW 타입 고객 전용 Processor
 *
 * - NEW 타입만 처리, 나머지는 필터링 (null 반환)
 * - 18세 미만 필터링
 * - 이름 대문자, 이메일 소문자 변환
 */
@Component
class NewCustomerProcessor : ItemProcessor<CustomerCsvRow, NewCustomer> {

    private val logger = LoggerFactory.getLogger(javaClass)

    override fun process(item: CustomerCsvRow): NewCustomer? {
        // 1. NEW 타입만 처리
        if (item.type.uppercase() != CustomerType.NEW.name) {
        ...
        logger.info("[NEW] 고객 처리: $firstName $lastName -> Secondary DB")
